...
export default function ShopDetails({ route }) {
    ...
    const getmembers = () => {
        // 192.168.0.121
        fetch(`http://172.20.10.2:5000/shops/${email}`)
            .then(res => res.json())
            .then(data => {
                setShop(data)
    ...
    const setStatusFilter = status => {
        ...
        if (status === 'About') {
            ...
            fetch(`http://172.20.10.2:5000/appointment/${email}`)
                .then(res => res.json())
                .then(data => {
                    setData(data)
                    setIsLoading(false)
    ...
    return (
        <ScrollView contentContainerStyle={[container, { backgroundColor: theme.background }]} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ marginHorizontal: 10 }}>
                ...
                {/* offer exist */}
                {shop.offer && <View style={styles.iconAndText}>
                    <View style={styles.icon}>
                        {/* star icon */}
                        <Entypo name='star' size={20} color={colors.darkOrange} />
                    </View>
                    <Text preset='info' style={{ marginTop: 5, }}> {shop.offer}</Text>
                </View>}
